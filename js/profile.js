document.addEventListener('DOMContentLoaded', () => {
    ...
    function openRightPanel(type) {
        ...
        if (btnFoldPanel) btnFoldPanel.classList.remove('hidden');
    ...
    function closeRightPanel() {
        ...
        if (btnFoldPanel) btnFoldPanel.classList.add('hidden');
    ...
    if (btnFoldPanel) {
        btnFoldPanel.addEventListener('click', closeRightPanel);
    }
    ...
    const cssHumanoid = document.querySelector('.css-humanoid');
    ...
    if (btnConfirmMeasure) {
        btnConfirmMeasure.addEventListener('click', (e) => {
            ...
            if (val && currentMeasurePartName) {
                ...
                const direction = leftSides.includes(currentMeasurePartId) ? 'direction-left' : 'direction-right';

                let existingCallout = document.querySelector(`.data-callout.for-${currentMeasurePartId}`);
                if (existingCallout) {
                    existingCallout.querySelector('.callout-text').innerHTML = `${currentMeasurePartName}: ${val}cm <span class="callout-delete" data-part="${currentMeasurePartId}">×</span>`;
                } else {
                    const callout = document.createElement('div');
                    callout.className = `data-callout ${direction} for-${currentMeasurePartId}`;
                    callout.innerHTML = `<div class="callout-line"></div><div class="callout-text">${currentMeasurePartName}: ${val}cm <span class="callout-delete" data-part="${currentMeasurePartId}">×</span></div>`;
                    if (cssHumanoid) cssHumanoid.appendChild(callout);
                }
    ...
    document.querySelector('.human-figure-container')?.addEventListener('click', (e) => {
        if (e.target.classList.contains('callout-delete')) {
            e.preventDefault();
            e.stopPropagation();
            const partId = e.target.getAttribute('data-part');
            const callout = document.querySelector(`.data-callout.for-${partId}`);
            if (callout) {
                callout.remove();
            }
            return;
